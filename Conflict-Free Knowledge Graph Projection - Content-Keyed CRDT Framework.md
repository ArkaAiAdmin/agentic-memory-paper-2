...
- **Scalability:** CK-CRDT merge is $O(N)$ for partitioning $N$ operations by key plus $O(M)$ for representative selection across $M$ distinct keys. When $M \approx N$ (no dedup), the framework provides no benefit over standard set-based CRDTs. The approach is most beneficial when $M \ll N$ (high dedup ratio). Empirical measurements on the reference implementation (`ck_crdt.py`, pure in-memory merge + dedup, no SQLite I/O) show throughput of 389K ops/s at 1K, degrading to 367K ops/s at 10M (1.06x) — nearly constant. Paper 1's implementation (`crdt_projection.py`, including SQLite I/O and full pipeline) shows 191K→136K ops/s, confirming the algorithmic complexity is $O(N)$ in both cases; the difference is I/O overhead, not algorithmic. At 575 bytes/op, 10M ops requires ~5.8 GB memory.
...
3. **Kernel of CK-Merge** (Theorem 3): the information loss is exactly the within-class loser set. **Theorem 3b** proves this is a tight lower bound — any K1-K3-compliant merge must discard at least $|O| - |\kappa(O)|$ operations.
