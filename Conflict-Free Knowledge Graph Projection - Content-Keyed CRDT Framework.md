...
- **Scalability:** CK-CRDT merge is $O(N)$ for partitioning $N$ operations by key plus $O(M)$ for representative selection across $M$ distinct keys. When $M \approx N$ (no dedup), the framework provides no benefit over standard set-based CRDTs. The approach is most beneficial when $M \ll N$ (high dedup ratio). Empirical measurements on a MacBook Pro confirm linear scaling: 100K ops in 0.4s, 1M in 5.4s, 10M in 61.6s — constant throughput of 0.2M ops/s. At 575 bytes/op, the in-memory reference implementation is memory-bound at ~5.5 GB for 10M ops. A billion operations would require ~536 GB (memory-bound, not compute-bound); a disk-backed streaming implementation handling the same O(N) algorithm would process 1B ops in ~83 minutes at the same throughput.
